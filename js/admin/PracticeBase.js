...
* @Last Modified time: 2018-06-09 23:56:04
*/
//获取目录
...
function buildTable(categoryLeafName) {
	var columns =[{"title":""},{"title":"序号"},{"title":"实习基地"},{"title":"对接老师"},{"title":"实习地点"},{"title":"对接时间"},{"title":"信息完整度"}];
...
function scienceTableDataAjax() {
	$.ajax({
		type: "get",
		url: "http://123.206.190.167:8080/dissertation/PracticeBase",
		data: {
		},
		async: true,
		dataType: "json",
...
function setTableData(obj) {
	$('#实习基地').dataTable().fnClearTable();
	var option_array = [];
	var option_array2 = [];
	$.each(obj.data, function(index, item) {
		if ($.inArray(item.contactTeacher, option_array)<0) {
			option_array.push(item.contactTeacher)
			$("#th3").append("<option value='"+item.contactTeacher+"'>"+item.contactTeacher+"</option>"); 
		};
		if ($.inArray(item.address, option_array2)<0) {
			option_array2.push(item.address)
			$("#th4").append("<option value='"+item.address+"'>"+item.address+"</option>"); 
		};
		order = index + 1;
...
// 选中序号构造下载url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/assetsSelected?categoryLeafName=实习基地' +
...
function inportResult() {
	var columns =[{"title":"实习基地名称"},{"title":"实习基地类型"},{"title":"第一责任人"},{"title":"立项时间"}];
	//给table加一个id 是二级目录的名字
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#实习基地').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
