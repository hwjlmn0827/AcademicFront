...
* @Date:   2017-12-26 03:13:01
* @Last Modified by:   Administrator
* @Last Modified time: 2018-06-09 10:57:58
*/
...
function buildTable(categoryLeafName) {
	var columns =[{"title":""},{"title":"序号"},{"title":"科研项目名称"},{"title":"第一作者"},{"title":"刊物级别"},{"title":"发表时间"},{"title":"信息完整度"}];
...
function setTableData(obj) {
	...
	$.each(obj.data, function(index, item) {
		...
		if ($.inArray(item.arrangement, option_array2)<0) {
			option_array2.push(item.arrangement)
			$("#th4").append("<option value='"+item.arrangement+"'>"+item.arrangement+"</option>"); 
		};
		order = index + 1;
		...
		$('#'+item.categoryLeafName).dataTable().fnAddData([
			'<input type="checkbox">',
			'<span>'+order+'</span>',
			'<a href="AcademicPapersDetail.html" id="'+item.id+'">'+item.name+'</a>',
...
// 选中序号构造url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/scientificProjectSelected?categoryLeafName=学术论文' +
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#学术论文').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
