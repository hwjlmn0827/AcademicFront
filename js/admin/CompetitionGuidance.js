...
* @Last Modified time: 2018-06-10 00:03:57
*/
//获取目录
...
function buildTable(categoryLeafName) {
	var columns =[{"title":""},{"title":"序号"},{"title":"竞赛名称"},{"title":"指导老师"},{"title":"竞赛级别"},{"title":"竞赛时间"},{"title":"信息完整度"}];
...
function scienceTableDataAjax() {
	$.ajax({
		type: "get",
		url: "http://123.206.190.167:8080/dissertation/RaceGuide",
		data: {
		},
		async: true,
		dataType: "json",
...
function setTableData(obj) {
	$('#竞赛指导').dataTable().fnClearTable();
	var option_array = [];
	var option_array2 = [];
	$.each(obj.data, function(index, item) {
		if ($.inArray(item.guideTeacher, option_array)<0) {
			option_array.push(item.guideTeacher)
			$("#th3").append("<option value='"+item.guideTeacher+"'>"+item.guideTeacher+"</option>"); 
		};
		if ($.inArray(item.raceLevel, option_array2)<0) {
			option_array2.push(item.raceLevel)
			$("#th4").append("<option value='"+item.raceLevel+"'>"+item.raceLevel+"</option>"); 
		};
		order = index + 1;
		...
		$('#'+item.categoryLeafName).dataTable().fnAddData([
			'<input type="checkbox">',
			'<span>'+order+'</span>',
			'<a href="CompetitionGuidanceDetail.html?id='+item.id+'" id="'+item.id+'">'+item.name+'</a>',
			'<span>'+item.guideTeacher+'</span>',
			'<span>'+item.raceLevel+'</span>',
			'<span>'+item.date+'</span>',
...
// 选中序号构造下载url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/assetsSelected?categoryLeafName=竞赛指导' +
	'&uniqueName=教学平台&fileName=竞赛指导.xlsx&ids='
	var ids = ""
...
function inportResult() {
	var columns =[{"title":"竞赛指导名称"},{"title":"竞赛指导类型"},{"title":"第一责任人"},{"title":"立项时间"}];
	//给table加一个id 是二级目录的名字
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#竞赛指导').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
