...
* @Date:   2018-06-09 11:01:46
* @Last Modified by:   Administrator
* @Last Modified time: 2018-06-09 11:27:49
*/
...
function buildTable(categoryLeafName) {
	var columns =[{"title":""},{"title":"序号"},{"title":"项目名称"},{"title":"第一责任人"},{"title":"项目级别"},{"title":"立项时间"},{"title":"信息完整度"}];
...
//填充数据
function studentTableDataAjax() {
	$.ajax({
		type: "get",
		url: "http://123.206.190.167:8080/dissertation/Project",
		data: {
		},
		async: true,
		dataType: "json",
...
function setTableData(obj) {
	...
	$.each(obj.data, function(index, item) {
		...
		if ($.inArray(item.lavel, option_array)<0) {
			option_array.push(item.lavel)
		...
		$('#'+item.categoryLeafName).dataTable().fnAddData([
			'<input type="checkbox">',
			'<span>'+order+'</span>',
			'<a href="StudentProjectDetail.html?id='+item.id+'" id="'+item.id+'">'+item.name+'</a>',
...
// 选中序号构造下载url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/assetsSelected?categoryLeafName=项目' +
	'&uniqueName=学生科研&fileName=项目.xlsx&ids='
	var ids = ""
...
function inportResult() {
	var columns =[{"title":"项目名称"},{"title":"项目类型"},{"title":"第一责任人"},{"title":"立项时间"}];
	//给table加一个id 是二级目录的名字
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#项目').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
