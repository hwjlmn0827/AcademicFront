...
function buildTable(categoryLeafName) {
	var columns =[{"title":""},{"title":"序号"},{"title":"培训名称"},{"title":"参加人"},{"title":"职称"},{"title":"培训时间"},{"title":"信息完整度"}];
...
function scienceTableDataAjax() {
	$.ajax({
		type: "get",
		url: "http://123.206.190.167:8080/dissertation/TeacherTrain",
		data: {
		},
		async: true,
		dataType: "json",
...
function setTableData(obj) {
	$('#教师培训').dataTable().fnClearTable();
	var option_array = [];
	var option_array2 = [];
	$.each(obj.data, function(index, item) {
		if ($.inArray(item.participantNames, option_array)<0) {
			option_array.push(item.participantNames)
			$("#th3").append("<option value='"+item.participantNames+"'>"+item.participantNames+"</option>"); 
		};
		if ($.inArray(item.jobNam, option_array2)<0) {
			option_array2.push(item.jobNam)
			$("#th4").append("<option value='"+item.jobNam+"'>"+item.jobNam+"</option>"); 
		};
		order = index + 1;
		...
		$('#'+item.categoryLeafName).dataTable().fnAddData([
			'<input type="checkbox">',
			'<span>'+order+'</span>',
			'<a href="TeacherTrainingDetail.html?id='+item.id+'" id="'+item.id+'">'+item.name+'</a>',
			'<span>'+item.participantNames+'</span>',
			'<span>'+item.jobNam+'</span>',
			'<span>'+item.date+'</span>',
...
// 选中序号构造下载url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/assetsSelected?categoryLeafName=教师培训' +
	'&uniqueName=教学平台&fileName=教师培训.xlsx&ids='
	var ids = ""
...
function inportResult() {
	var columns =[{"title":"教师培训名称"},{"title":"教师培训类型"},{"title":"第一责任人"},{"title":"立项时间"}];
	//给table加一个id 是二级目录的名字
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#教师培训').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
