...
* @Last Modified time: 2018-06-10 00:22:45
*/
//获取目录
...
function scienceTableDataAjax() {
	$.ajax({
		type: "get",
		url: "http://123.206.190.167:8080/dissertation/Textbook",
		data: {
		},
		async: true,
		dataType: "json",
...
function setTableData(obj) {
	...
	$.each(obj.data, function(index, item) {
		...
		$('#'+item.categoryLeafName).dataTable().fnAddData([
			'<input type="checkbox">',
			'<span>'+order+'</span>',
			'<a href="TeachingMaterialDetail.html?id='+item.id+'" id="'+item.id+'">'+item.name+'</a>',
...
// 选中序号构造下载url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/assetsSelected?categoryLeafName=教材' +
...
function inportResult() {
	var columns =[{"title":"教材名称"},{"title":"教材类型"},{"title":"第一责任人"},{"title":"立项时间"}];
	//给table加一个id 是二级目录的名字
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#教材').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
