...
* @Last Modified time: 2018-06-09 23:48:17
*/
//获取目录
...
function buildTable(categoryLeafName) {
	var columns =[{"title":""},{"title":"序号"},{"title":"实验室"},{"title":"指导老师"},{"title":"实验室地址"},{"title":"创立时间"},{"title":"信息完整度"}];
...
function setTableData(obj) {
	...
	$.each(obj.data, function(index, item) {
		...
		$('#'+item.categoryLeafName).dataTable().fnAddData([
			'<input type="checkbox">',
			'<span>'+order+'</span>',
			'<a href="LaboratoryDetail.html?id='+item.id+'" id="'+item.id+'">'+item.name+'</a>',
...
// 选中序号构造下载url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/assetsSelected?categoryLeafName=实验室' +
...
function inportResult() {
	var columns =[{"title":"实验室名称"},{"title":"实验室类型"},{"title":"第一责任人"},{"title":"立项时间"}];
	//给table加一个id 是二级目录的名字
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#实验室').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
