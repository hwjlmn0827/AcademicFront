...
* @Date:   2017-12-26 03:06:09
* @Last Modified by:   Administrator
* @Last Modified time: 2018-06-09 10:27:10
*/
...
function buildTable(categoryLeafName) {
	var columns =[{"title":""},{"title":"序号"},{"title":"获奖科研成果名称（种类）"},{"title":"第一责任人"},{"title":"获奖等级"},{"title":"获奖时间"},{"title":"信息完整度"}];
...
function RewardTableDataAjax() {
	$.ajax({
		type: "get",
		url: "http://123.206.190.167:8080/dissertation/Winning",
		data: {
		},
		async: true,
		dataType: "json",
...
function setTableData(obj) {
	...
	$.each(obj.data, function(index, item) {
		...
		if ($.inArray(item.level, option_array2)<0) {
			option_array2.push(item.level)
			$("#th4").append("<option value='"+item.level+"'>"+item.level+"</option>"); 
		};
		order = index + 1;
		...
		$('#'+item.categoryLeafName).dataTable().fnAddData([
			'<input type="checkbox">',
			'<span>'+order+'</span>',
			'<a href="RewardDetail.html" id="'+item.id+'">'+item.name+'</a>',
...
// 选中序号构造url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/scientificProjectSelected?categoryLeafName=获奖' +
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#获奖').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
