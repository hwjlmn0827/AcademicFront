...
function buildTable(categoryLeafName) {
	var columns =[{"title":""},{"title":"序号"},{"title":"学术论文名称"},{"title":"第一作者"},{"title":"刊物级别"},{"title":"发表时间"},{"title":"信息完整度"}];
...
function scienceTableDataAjax() {
	$.ajax({
		type: "get",
		url: "http://123.206.190.167:8080/dissertation/Academic",
		data: {
		},
		async: true,
		dataType: "json",
...
function setTableData(obj) {
	...
	$.each(obj.data, function(index, item) {
		...
		if ($.inArray(item.lavel, option_array2)<0) {
			...
			$("#th3").append("<option value='"+item.lavel+"'>"+item.lavel+"</option>"); 
		};
		order = index + 1;
...
// 选中序号构造下载url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/assetsSelected?categoryLeafName=学术论文' +
	'&uniqueName=学生科研&fileName=学术论文.xlsx&ids='
	var ids = ""
...
function inportResult() {
	var columns =[{"title":"学术论文名称"},{"title":"学术论文类型"},{"title":"第一责任人"},{"title":"立项时间"}];
	//给table加一个id 是二级目录的名字
