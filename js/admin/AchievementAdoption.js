...
* @Date:   2017-12-26 02:35:41
* @Last Modified by:   Administrator
* @Last Modified time: 2018-06-09 10:34:37
*/
/*
* @Author: Administrator
* @Date:   2017-11-29 14:41:23
* @Last Modified by:   Administrator
* @Last Modified time: 2017-12-26 02:26:56
*/
...
function buildTable(categoryLeafName) {
	var columns =[{"title":""},{"title":"序号"},{"title":"采纳成果名称"},{"title":"第一完成人"},{"title":"采纳单位"},{"title":"采纳时间"},{"title":"信息完整度"}];
...
function AchievementAdoptionTableDataAjax() {
	$.ajax({
		type: "get",
		url: "http://123.206.190.167:8080/dissertation/Achievements",
		data: {
		},
		async: true,
		dataType: "json",
...
function setTableData(obj) {
	...
	$.each(obj.data, function(index, item) {
		...
		$('#'+item.categoryLeafName).dataTable().fnAddData([
			'<input type="checkbox">',
			'<span>'+order+'</span>',
			'<a href="AchievementAdoptionDetail.html?id='+item.id+'" id="'+item.id+'">'+item.name+'</a>',
...
// 选中序号构造url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/scientificProjectSelected?categoryLeafName=成果采纳' +
...
function inportResult() {
	var columns =[{"title":"项目名称"},{"title":"科研项目类型"},{"title":"第一责任人"},{"title":"立项时间"}];
	//给table加一个id 是二级目录的名字
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#成果采纳').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
