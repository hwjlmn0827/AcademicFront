...
* @Last Modified time: 2018-06-10 00:09:00
*/
//获取目录
...
function buildTable(categoryLeafName) {
	var columns =[{"title":""},{"title":"序号"},{"title":"项目名称"},{"title":"第一责任人"},{"title":"获奖等级"},{"title":"获奖时间"},{"title":"信息完整度"}];
...
function scienceTableDataAjax() {
	$.ajax({
		type: "get",
		url: "http://123.206.190.167:8080/dissertation/StudentPatent",
		data: {
		},
		async: true,
		dataType: "json",
...
function setTableData(obj) {
	...
	$.each(obj.data, function(index, item) {
		...
		if ($.inArray(item.awardLevel, option_array2)<0) {
			option_array2.push(item.awardLevel)
			$("#th4").append("<option value='"+item.awardLevel+"'>"+item.awardLevel+"</option>"); 
		};
		order = index + 1;
		...
		$('#'+item.categoryLeafName).dataTable().fnAddData([
			'<input type="checkbox">',
			'<span>'+order+'</span>',
			'<a href="TeacherAwardDetail.html?id='+item.id+'" id="'+item.id+'">'+item.name+'</a>',
...
function makeURL () {
	...
	'&uniqueName=教学平台&fileName=教师获奖.xlsx&ids='
	var ids = ""
...
function inportResult() {
	var columns =[{"title":"教师获奖名称"},{"title":"教师获奖类型"},{"title":"第一责任人"},{"title":"立项时间"}];
	//给table加一个id 是二级目录的名字
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#教师获奖').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
