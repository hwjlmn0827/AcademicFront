...
function scienceTableDataAjax() {
	$.ajax({
		type: "get",
		url: "http://123.206.190.167:8080/dissertation/Competition",
		data: {
		},
		async: true,
		dataType: "json",
...
// 选中序号构造下载url
function makeURL () {
	var url = 'http://123.206.190.167:8080/dissertation/excel/assetsSelected?categoryLeafName=竞赛获奖' +
	'&uniqueName=学生科研&fileName=竞赛获奖.xlsx&ids='
	var ids = ""
...
function inportResult() {
	var columns =[{"title":"竞赛获奖名称"},{"title":"竞赛获奖类型"},{"title":"第一责任人"},{"title":"立项时间"}];
	//给table加一个id 是二级目录的名字
...
$(document).on('click', '.sureDelete_dataYes', function() {
	...
	$.ajax({
		...
		success: function(data) {
			console.log(data)
			if (data.err) {
				$('#竞赛获奖').DataTable().rows('.tr_selected').remove().draw(false);
			};
		},
		Error: function() {
			alert("服务器出错");
